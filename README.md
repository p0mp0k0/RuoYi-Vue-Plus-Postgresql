## 修改简介
[![码云Gitee](https://gitee.com/p0mp0k0/RuoYi-Vue-Plus-Postgresql/badge/star.svg?theme=blue)](https://gitee.com/JavaLionLi/RuoYi-Vue-Plus)
...
4. 增加两个Postgresql函数(list_table,list_column)，对应GenTable和genTableColumn查询系统表数据，并修改Mapper xml中的sql语句
5. 修改GenTable的Mapper文件适配Postgresql
6. 修改Domain，修改@TableId增加type = IdType.AUTO，增加@KeySequence
7. 修改vm中的domain和sql语句
8. 修改insertAll注入类中主键字段过滤以适配Postgresql批量插入时主键的自动生成
9. 增加了Postgresql的bytea类型
>测试验证
1. 系统原有功能已经过验证测试
2. 测试数据库版本是14.0.1
>使用说明
1. 按照RuoYi-Vue-Plus的使用说明进行操作
2. 需要将Postgresql额外增加的一些View、Function、Cast等导入，见script路径下的pg.sql
>补充

由于事情太多，有一段时间一直没看RuoYi-Vue-Plus的变化，所以也就一直没更新，等到有时间去看的时候发现RuoYi-Vue-Plus拆分成了多模块版本模式，而且其中的部分组件进行了替换，于是就更没时间梳理变化了。
